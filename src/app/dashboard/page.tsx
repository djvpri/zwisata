...
export default function Dashboard() {
  ...
  const cards = [
    { label: 'Wahana', value: stats.wahana, color: 'bg-blue-500', icon: <i className="bi bi-signpost-split text-2xl"></i> },
    { label: 'Tiket', value: stats.tiket, color: 'bg-green-500', icon: <i className="bi bi-ticket-perforated text-2xl"></i> },
    { label: 'Pesanan', value: stats.pesanan, color: 'bg-amber-500', icon: <i className="bi bi-clipboard-data text-2xl"></i> },
  ]

  return (
    <div className="min-h-screen">
      <header className="border-b px-4 py-3 bg-card flex items-center justify-between sticky top-0 z-20">
        <div className="flex items-center gap-2 font-bold text-lg"><i className="bi bi-signpost-split"></i> ZWisata</div>
      ...
      <main className="max-w-6xl mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <a href="/dashboard/wahana" className="border rounded-xl p-4 hover:bg-muted transition flex items-center gap-3">
            <i className="bi bi-signpost-split text-2xl"></i>
